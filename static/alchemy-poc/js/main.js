...
var playable = [];
var inventory = [ 'Art', 'Writing', 'News', 'Something Else' ];

// List of possible combos to create new elements
var COMBO = {
	'Art'   : { info: "An ancient classical element" },
	'Writing' : { info: "An ancient classical element" },
	'News'  : { info: "An ancient classical element" },
	'Something Else' : { info: "An ancient classical element" },
	
	'Design' : { info: "It's muddy.", needs: [ 'Art', 'Reveal' ] },
	'Painting' : { info: "It's muddy.", needs: [ 'Art', 'Reveal' ] },
	'Drawing' : { info: "It's muddy.", needs: [ 'Art', 'Reveal' ] },
	'Advertising' : { info: "It's muddy.", needs: [ 'Art', 'Writing' ] },
...
var DEFAULT_INVENTORY = [ 'Art', 'Writing', 'News', 'Something Else' ];
...
var dragged_twice = {
	stack: ".elem",
	drag: function( ev, ui ) {
		// Change color of background when dragging to delete
...
function elem_was_moved( el, id ) {
	...
	$( playable ).each( function( i, e ) {
		...
		//if( e.id != id ) {
			...
			if(elem_is_playable("#"+e.id)) {
				// Element is in the "playable" area, check if any others are a match
				if(e.reveal) {
					var new_elems = elem_is_combo( e.name, 'Reveal' );
				}
				if( e.id != id ) {
					var new_elems = elem_is_combo( elem_get_name( el ), e.name );
				}

				// Need something to check if already exists
				if( new_elems && e.name != name) {
					// OMG! Combo!
...
function elem_add_to_play_area( new_elem, top, left ) {
	top = elem_return_position_in_viewport(top);
	left = left + elem_return_position_in_viewport(left);
...
function elem_return_position_in_viewport (dimension) {
	min = 0;
	max = 1680; // default
	if (dimension === "top") {
		alert("top");
...
function elem_add_to_inventory_area( new_elem, do_it_fast ) {
	...
	var top = elem_return_position_in_viewport(top);
	var left = elem_return_position_in_viewport(left);
	$(newdiv).css({
		"position": "absolute",
		"top" : top,
		"left": left
	}).show( "drop", { direction: "up" }, speed );
	elem_update_inventory_tooltips();
...
function elem_update_playable() {
	...
	$('#play .elem').each( function( i, e ) {
		...
		var left = parseInt( p.left );
		var id = $(e).attr( 'id' );
		playable.push( { id: id, name: name, top: top, left: left, reveal: "true" } );
	});
}
